...
private def text_value(node, trim_blocks = false, lstrip_blocks = false)
  Crinja::Visitor::Renderer.trim_text(node.as(Crinja::Node::Text), trim_blocks, lstrip_blocks)
end

describe "whitespace" do
  it "trims whitespace after tag" do
    template = parse(%(<div>\n    {% if true -%}\n        yay\n    {% endif %}\n</div>))
    text_value(template.root.children[1].children[0]).should eq("        yay\n    ")
  end

  it "trims before tag" do
    template = parse(%(<div>\n    {% if true %}\n        yay\n    {%- endif %}\n</div>))
    text_value(template.root.children[1].children[0]).should eq("\n        yay\n")
  end

  it "trims before tag with lstrip blocks" do
    template = parse(%(<div>\n    {% if true %}\n        yay\n    {%- endif %}\n</div>))
    text_value(template.root.children[1].children[0], false, true).should eq("\n        yay")
  end
  ...
  it "trims empty text left side correctly" do
    ...
    text_value(template.root.children[0].children[0]).should eq("\n")
    text_value(template.root.children[0].children[2]).should eq("  ")
  ...
  it "trims around blocks" do
    ...
    text_value(template.root.children[1].children[0]).should eq("    ")
    text_value(template.root.children[1].children[3]).should eq("")
  end
