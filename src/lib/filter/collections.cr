module Crinja::Filter
  ...
  Crinja.filter({slices: 2, fill_with: nil}, :slice) do
    ...
    if target.sequence?
      ...
      target.each_with_index do |item|
        slice << item
  ...
  Crinja.filter({attribute: nil, start: 0}, :sum) do
    ...
    target.raw_each do |raw|
      unless attribute.nil?
        raw = Resolver.resolve_dig(attribute, raw)
      end
  ...
  Crinja.filter({attribute: UNDEFINED}, :groupby) do
    ...
    target.each do |item|
      ...
      if h.has_key?(value)
        h[value].as_a.push(item)
      else
        h[value] = Crinja::Value.new [item]
      end
    end
    h
  end
