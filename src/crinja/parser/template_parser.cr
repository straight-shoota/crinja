module Crinja::Parser
  class TemplateParser
    ...
    @trim_left = false
    @left_is_block = false
    @last_sibling_fixed : FixedString?
    ...
    private def parse_node_list(block = false)
      ...
      while true
        ...
        when Kind::EOF
          ...
          @last_sibling_fixed = if node.is_a?(FixedString)
                                  node
                                else
                                  nil
                                end

          if node.is_a?(EndTagNode)
            if @stack.size > 0
              parent, expected_end_name = @stack.last
      ...
      NodeList.new(nodes, block).at(start_location, end_location)
    end
    ...
    private def parse_print_statement
      ...
      PrintStatement.new(expression).at(start_location, end_location)
    end
    ...
    private def parse_tag
      ...
      if tag.is_a?(Tag::EndTag)
        node = EndTagNode.new(name_token.value, arguments).at(start_location, end_location)

        return node
      else
        block = NodeList.new([] of TemplateNode, true)

        node = TagNode.new(name_token.value, arguments, block, nil).at(start_location, end_location)
