  ...
  class ExpressionParser
    ...
    def parse_expressions(expected_end_token : Kind = Kind::EOF)
      ...
      while true
        case current_token.kind
        when expected_end_token
          return Expressions.new(expressions).at(current_token.location)
        else
    ...
    parse_operator :equal_not, :less_greater, EQUAL, NOT_EQUAL, NOT do
      ComparisonExpression.new operator, left, right
    end
      ...
      while true
        ...
        when Kind::PIPE, Kind::TEST
          ...
          identifier = if_token(Kind::NONE) do
            IdentifierLiteral.new(current_token.value).at(current_token.location)
          ...
          if is_test
            left = TestExpression.new(left, identifier, call.argumentlist, call.keyword_arguments).at(left, call)

            if not_location
              left = UnaryExpression.new("not", left).at(not_location)
            end
          else
            left = FilterExpression.new(left, identifier, call.argumentlist, call.keyword_arguments).at(left, call)
          end
        else
          return left
        end
    ...
    private def parse_unary_expression
      ...
      if current_token.kind == Kind::OPERATOR
        ...
        when Symbol::OP_TIMES
          ...
          next_token
          value = parse_unary_expression
          return SplashOperator.new(value).at(start_location, value.location_end)
        end
      end
      ...
      while true
        ...
        when Kind::POINT
          ...
          assert_token Kind::IDENTIFIER do
            member = IdentifierLiteral.new(current_token.value).at(current_token.location)
            next_token
          end
    ...
    private def parse_call_expression(identifier, with_parenthesis = true)
      ...
      kwargs = if keyword
                 ...
                 Hash(IdentifierLiteral, ExpressionNode).new
               end

      end_location = current_token.location
      expect Kind::RIGHT_PAREN if with_parenthesis
      CallExpression.new(identifier, args, kwargs).at(identifier.location_start, end_location)
    end
    ...
    private def parse_literal
      ...
      when Kind::INTEGER
        node = IntegerLiteral.new(current_token.value.to_i64).at(current_token.location)
        next_token
      when Kind::FLOAT
        node = FloatLiteral.new(current_token.value.to_f64).at(current_token.location)
        next_token
      when Kind::STRING
        node = StringLiteral.new(current_token.value).at(current_token.location)
        next_token
      when Kind::BOOL
        node = BooleanLiteral.new(current_token.value.downcase == "true").at(current_token.location)
        next_token
      when Kind::NONE
        node = NullLiteral.new.at(current_token.location)
        next_token
      when Kind::LEFT_BRACKET
    ...
    private def parse_identifier
      node = IdentifierLiteral.new(current_token.value).at(current_token.location)
      next_token
      node
    end

    private def parse_expression_list(end_tokens : Array(Kind))
      exps = [] of ExpressionNode
      start_location = current_token.location
    ...
    def parse_keyword_list(end_tokens : Array(Kind) = [Kind::EOF], keyword_separator_token : Kind = Kind::KW_ASSIGN, keyword = nil)
      ...
      while should_read
        ...
        if keyword.is_a?(IdentifierLiteral)
    ...
    private def parse_tuple_literal
      ...
      return TupleLiteral.new(exps.children).at(start_location, end_location)
    end
    ...
    private def parse_dict_literal
      ...
      hash = Hash(ExpressionNode, ExpressionNode).new
      ...
      return DictLiteral.new(hash).at(start_location, end_location)
    end

    private def parse_identifier_list
      list = [] of IdentifierLiteral
